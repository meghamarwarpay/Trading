...
export const Hero = () => {
  ...
  return (
    <div className="relative bg-[#0a0b2e] overflow-hidden">
      <motion.div 
        className="absolute top-0 right-0 w-1/2 h-full bg-yellow-400 transform origin-top-right"
      ...
      <div className="container relative z-10 pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center min-h-[70vh]">
          <div className="text-white space-y-6 p-8">
            ...
            >
              A SEBI Registered Research Analyst(INH000016472) with 6+ years of experience.
            </motion.p>
            <motion.div
            ...
            >
              {carouselItems.map((item, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.5 }}
                  className={`absolute ${currentItem === index ? 'block' : 'hidden'}`}
                >
   ...
   <div className="relative z-10 pb-8 "> {/* Move button upwards using -mt-8 */}
  <motion.div
    initial={{ x: -100, opacity: 0 }}
    animate={{ x: 0, opacity: 1 }}
...
<div className="absolute bottom-0 left-0 w-full mt-20" > {/* Adjust margin to control the position */}
